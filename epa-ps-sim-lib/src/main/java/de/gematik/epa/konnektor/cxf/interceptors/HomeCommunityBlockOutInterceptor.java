            ...
            .or(
                () ->
                    Optional.ofNullable(message.getHeader(CONTEXT_HEADER_NAME))
                        ...
                        .map(obj -> MiscUtils.safeCast(obj, JAXBElement.class))
                        .map(jxbel -> MiscUtils.safeCast(jxbel.getValue(), ContextHeader.class)))
            .or(
                () ->
    ...
    if (Objects.nonNull(contextHeader)) {
      ...
      var soapHeader =
          new Header(
              jaxbElement.getName(),
              jaxbElement,
              new JAXBDataBinding(jaxbElement.getDeclaredType()));
