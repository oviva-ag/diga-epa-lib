...
public class AuthSignatureServiceClient {
  ...
  private static final String SIGNATURE_TYPE = "urn:bsi:tr:03111:ecdsa";

  private static final ECNamedCurveParameterSpec BP256_SPEC =
  ...
  /**
   * @param bytesToSign 32 bytes of data to sign (e.g. a SHA-256 hash)
   * @return the signed bytes in the <a
   *     href="https://datatracker.ietf.org/doc/html/rfc7518#section-3.4">concatenated form</a>
   */
  public byte[] signAuthHash(String cardHandle, byte[] bytesToSign) {
    ...
    var req = buildExternalAuthenticate(cardHandle, bytesToSign);
    ...
    if (!SIGNATURE_TYPE.equals(signatureType)) {
      throw new KonnektorException(
          "signature type unexpected, want %s but got %s, is the connector ECC capable?"
              .formatted(SIGNATURE_TYPE, signatureType));
    }
  ...
  public static byte[] convertDerEcdsaToConcated(byte[] derSignature) {
    ...
    try {
      var signInt = StandardDSAEncoding.INSTANCE.decode(BP256_SPEC.getN(), derSignature);
      var r = signInt[0];
      var s = signInt[1];
  ...
  private ExternalAuthenticate buildExternalAuthenticate(String cardHandle, byte[] bytesToSign) {
