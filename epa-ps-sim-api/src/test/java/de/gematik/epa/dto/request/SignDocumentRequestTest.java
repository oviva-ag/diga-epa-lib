...
class SignDocumentRequestTest {
  ...
  void signatureTypeOfUriTest() {
    var signatureType = assertDoesNotThrow(() -> SignatureType.of(SignatureType.CMS.uri()));

    assertEquals(SignatureType.CMS, signatureType);
  }

  @Test
  void signatureTypeOfUnkownUriTest() {
    var uri = URI.create("http://wrong/uri");
    var exception = assertThrows(IllegalArgumentException.class, () -> SignatureType.of(uri));
  ...
  void signatureTypeOfNameTest() {
    var signatureType = assertDoesNotThrow(() -> SignatureType.of(SignatureType.CMS.name()));
  ...
  void signatureTypeOfUnkownNameTest() {
    var name = "DOC";
    var exception = assertThrows(IllegalArgumentException.class, () -> SignatureType.of(name));
        ...
        assertDoesNotThrow(
            () ->
                new SignDocumentRequest(
                    "the document".getBytes(StandardCharsets.UTF_8), true, SignatureAlgorithm.ECC));
  }
}
